package com.offerdyne.repository;

import com.offerdyne.entity.TranscriptHistory;
import org.springframework.data.jpa.repository.JpaRepository;
import java.util.List;

public interface TranscriptHistoryRepository extends JpaRepository<TranscriptHistory, Long> {
    List<TranscriptHistory> findByTranscriptIdOrderByArchivedAtAsc(Long transcriptId);
    List<TranscriptHistory> findBySessionIdOrderByArchivedAtAsc(Long sessionId);
}
